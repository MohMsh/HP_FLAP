...
class Client:
    ...
    def join_cycle(self):
        ...
        exchange_messages_thread = ExchangeMessagesThread(buffer_size=self.buffer_size,
                                                          ...
                                                          random_secret=self.random_secret, HEToKs = self.HEToKs)
...
class ExchangeMessagesThread(threading.Thread):
    ...
    def __init__(self, buffer_size, receive_timeout, this_socket, receive_start_time, initial_key,
                 print_incoming_messages, print_sent_messages, x_train, y_train, x_test, y_test, random_secret, HEToKs):
        """
        class constructor
        :param buffer_size: size of exchanged message
        :param receive_timeout: time before client drops connection if no response from server
    ...
    def reply(self):
        ...
        if self.message_subject == "EncToKs":
            self.EncToKs = self.message_content["EncToKs"]
            self.HEToKs = self.message_content["HEToKs"]
        ...
        # if the message is entitled as 'Model', the training shall start
            ...
            if self.message_content["stage"]=="aggregated":
                ...
                # we should change the public key at this level so that it will be passed to the model_class
                # the next encryption round will be done with this key and decryption will be done with its related
                # private key
                self.public_key = self.HEToKs[self.message_content["index"]][1]

                # => load parameters to the available model
                model_class = local_model.local_model(self.local_model, self.public_key, self.private_key)
                self.local_model = deepcopy(model_class.set_parameters(parameters))


                self.private_key = self.HEToKs[self.message_content["index"]][2]
            ...
            # inform the user that the model training is complete
            ...
            # in this line, I should encrypt the parameters with HE
