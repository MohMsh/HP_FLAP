...
class server:
    ...
    def start(self):
        ...
        # create the Homomorphic Encryption Table of Keys (HEToKs) randomly based on the assigned size of keys table
        self.HEToKs = [[] for x in range(self.table_size)]
        for i in range(self.table_size):
            public_key, private_key = paillier.generate_paillier_keypair()
            self.HEToKs[i] = [i, public_key, private_key]

        self.lock.acquire()
        HEIndex.append(random.randint(0, len(self.HEToKs)-1))
        self.lock.release()

        # print("HE Table of Keys: ", self.HEToKs )
        # print("Current Index of HE: ", HEIndex[-1])
        # print("Keypair to be used: ", self.HEToKs[HEIndex[-1]])
...
class listen_thread(threading.Thread):
    ...
    def __init__(self, server_socket, minimum_clients, buffer_size, timeout, print_incoming_messages,
                 ...
                 x_test, y_test, global_rounds, current_rounds, table_size, HEToKs):
        """
        class constructor
        ...
        while True:
            try:
                ...
                communication_thread = communicate_thread(client_socket=client_socket, client_address=client_address,
                                                          ...
                                                          table_size=self.table_size, HEToKs=self.HEToKs)
...
class communicate_thread(threading.Thread):
    ...
    def __init__(self, client_socket, client_address, buffer_size, timeout, receive_start_time,
                 minimum_clients, print_incoming_messages, print_sent_messages, print_model_summary,
                 print_model_performance, aggregation_method, model, x_test, y_test, global_rounds, current_rounds,
                 table_size, HEToKs):
        """
        class constructor
        ...
        if type(received_message) is dict:
            ...
            if (self.message_subject is not None) and (self.message_content is not None):
                ...
                if self.message_subject == "Connect":
                    ...
                    self.response_content = {"EncToKs":self.EncToKs, "HEToKs":self.HEToKs}
                    ...
                    self.response = {"subject": self.response_subject,
                                     "content": {"model": self.model, "stage":"initial", "index": HEIndex[-1]}}
                    ...
                    """
                    try:
                        # client_model_parameters = self.message_content["content"]
                        client_model_parameters = self.message_content
                    ...
                    if len(clients_models_parameters) >= self.minimum_clients:
                        ...
                        global_model_class = global_model.global_model(clients_models_parameters, self.model,
                                                                       self.x_test, self.y_test, self.current_round,
                                                                       self.print_model_summary,
                                                                       self.print_model_performance,
                                                                       self.HEToKs[HEIndex[-1]][1])
                        # aggregate the models
                        aggregated_parameters = global_model_class.aggregate()
                        # self.model, aggregated_parameters = global_model_class.aggregate()
                        ...
                        if self.current_round > self.global_rounds:
                            ...
                            print("Sending from first loop")

                            for member in clients_list:
                                print("Sending from second loop")
                                member.sendall(pickle.dumps(self.response))
                            return True

                        else:
                            for member in clients_list:
                                # encrypted_response = self.encrypt(self.response)
                                # member.sendall(pickle.dumps(encrypted_response))
                                member.sendall(pickle.dumps(self.response))
                            return True
